class Service < ActiveRecord::Base
  ...
  def pretend_ago
    errors.add(:period_start, '本日より過去日の設定は出来ません') if period_start.nil? || period_start <= Date.today -1
    errors.add(:period_end, '終了日は開始日の後に設定してください') if period_end.nil? || period_end <= period_start - 2
  end
